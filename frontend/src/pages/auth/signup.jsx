...
const SignUp = () => {
 ...
 const { handleSubmit, register, reset, watch, formState: { errors, isValid } } = useForm({
            defaultValues: {
              fullName: "",
              email: "",
              password: "",
          ...
          const { mutate: signInMutation, isPending,  } = useMutation({
            mutationFn: async (formData) => await apiClient.signUp(formData),
            onSuccess: async () => {
              reset();
              await  queryClient.invalidateQueries({ queryKey: ["listAlbum"] })
              await queryClient.invalidateQueries({ queryKey: ["listSongs"] })
              navigate("/")
            },
            onError: (error) => {
  ...
  return (
    <section className="bg-white h-dvh w-full max-w-full flex flex-col md:flex-row items-start justify-center">
      ...
      <div className="flex-1 flex flex-col items-center justify-center h-full gap-5 p-12 px-5 lg:p-12 w-full">
        <h2 className="font-bold text-4xl w-full">Let{"'"}s get started</h2>
        ...
        <form className='w-full' onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex items-center gap-1.5 w-full mt-3">
              ...
              <Link to={"/signin"} className='text-blue-500 underline underline-offset-4 text-sm'>SignIn</Link>
            </div>
