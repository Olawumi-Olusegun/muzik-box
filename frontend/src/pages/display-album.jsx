...
const DisplayAlbum = () => {
  ...
  const { playWithId, songsData, albumsData } = usePlayerContext()
  ...
  return (
    ...
    <hr />
    {
     songsData?.length > 0 && songsData.map((song, index) => (
        <div onClick={() => playWithId(song.id)} key={song.id} className="grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] duration-300 cursor-pointer ">
